const footer = document.querySelector(".footer");

footer.innerHTML = `<section class="container-fluid  text-center">
        <article class="row my-4 my-md-2 pt-md-4">
          <div class="col-sm-12 col-md-6 mb-2 mb-md-0">    
            <h4> <a href="/index.html">Temporizador 🕑</a></h4>
          </div>
          
          <div class="col-sm-12 col-md-6">
            <h5 class="jost">Soporte</h5>
            <ul
              class="list-unstyled d-flex justify-content-center flex-column flex-md-row"
            >
              <li>
                <a class="jost" href="/temporizador/componentes/pages/nosotros.html">Nosotros</a>
              </li>
              <li>
                <a class="jost " href="/temporizador/componentes/pages/404.html">Contactános</a>
              </li>

              <li>
                <a class="jost" href="/temporizador/componentes/pages/404.html">Ayuda</a>
              </li>
            </ul>
          </div>
        </article>
      </section>
      <p class="text-center border-none montserrat my-0 mt-2">
        &copy; 2024. Todos los derechos reservados
      </p> `;